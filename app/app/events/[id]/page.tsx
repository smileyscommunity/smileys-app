...
export default async function AppEventDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const others = attendees.filter((a) => !a.isFriend)
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white rounded-2xl shadow-card overflow-hidden">
              ...
              <div className="p-8">
                {/* Premium / Members-only header banner */}
                {event.membersOnly && (
                  <div className="flex items-center gap-3 bg-violet-50 border border-violet-200 rounded-xl px-4 py-3 mb-5">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-violet-600 mt-0.5">This event is exclusively available to Smileys members.</div>
                    </div>
                ...
                {event.isPremium && !event.membersOnly && (
                  <div className="flex items-center gap-3 bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 mb-5">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-amber-600 mt-0.5">Members save ₺{event.price - (event.memberPrice ?? event.price)} on this event.</div>
                    </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                {[
                  ...
                  },
                  {
                    ...
                    label: 'Capacity',
                    value: `${event.totalSpots - event.spotsLeft} / ${event.totalSpots} joined`,
                  },
                ].map((detail) => (
            ...
            {/* Pricing breakdown (premium events) */}
            {(event.isPremium || event.membersOnly) && (
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex items-center justify-between p-4 rounded-xl bg-violet-50 border-2 border-violet-200">
                    ...
                    <div className="text-right">
                      ...
                      {event.price > 0 && event.memberPrice !== undefined && event.price > event.memberPrice && (
                  ...
                  {event.membersOnly && (
                    <div className="flex items-center gap-3 p-4 rounded-xl bg-gray-50 border border-dashed border-gray-300">
                      ...
                      <p className="text-sm text-gray-500">
                        Guest access is not available for this event.{' '}
                        <a href="#" className="text-violet-600 font-semibold hover:underline">Become a member</a> to join.
                      </p>
                    </div>
                  )}
                </div>
            ...
            {friends.length > 0 && (
              <div className="bg-white rounded-2xl shadow-card p-8">
                <div className="flex items-center gap-2 mb-5">
                  ...
                  <span className="ml-auto badge bg-amber-100 text-amber-700">{friends.length} friend{friends.length > 1 ? 's' : ''}</span>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {friends.map((f) => (
                    <div key={f.id} className="flex items-center gap-3 p-3 rounded-xl bg-amber-50 border border-amber-100">
                      <div
                        className="w-11 h-11 rounded-full flex items-center justify-center text-white text-sm font-bold shrink-0 ring-2 ring-white shadow-sm"
                        style={{ backgroundColor: f.color }}
                      >
                        {f.initials}
                      </div>
                      <div className="min-w-0 flex-1">
                        <div className="font-semibold text-sm text-gray-900 truncate">{f.name}</div>
                        <div className="text-xs text-gray-500 mt-0.5 truncate">{f.bio}</div>
                      </div>
                      <button className="shrink-0 text-xs font-semibold px-3 py-1.5 rounded-lg bg-white border border-amber-200 text-amber-700 hover:bg-amber-100 transition-colors">
                        Say hi
                      </button>
                    </div>
                  ))}
            ...
            <div className="bg-white rounded-2xl shadow-card p-8">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {others.map((a) => (
                  <div key={a.id} className="flex items-center gap-2.5 p-2.5 rounded-xl hover:bg-gray-50 transition-colors group">
                    <div
                ...
                {remainingCount > 0 && (
                  <div className="flex items-center gap-2.5 p-2.5 rounded-xl bg-gray-50">
                    ...
                    <div className="text-sm text-gray-400">more going</div>
                  </div>
                )}
              </div>
            </div>
          ...
          <div className="space-y-5">
            <div className="bg-white rounded-2xl shadow-card p-6 sticky top-24">
              ...
              {event.membersOnly ? (
                <>
                  <button className="inline-flex items-center justify-center gap-2 w-full px-6 py-3.5 bg-violet-600 hover:bg-violet-700 text-white font-semibold rounded-xl transition-all duration-200 shadow-sm mb-3 text-base">
                    Join as member
                  </button>
                  <button className="btn-secondary w-full justify-center text-sm py-2.5 mb-3">
                    Learn about membership
                  </button>
                </>
              ) : (
                <>
                  <button className="btn-primary w-full justify-center text-base py-3.5 mb-3">
                    {event.memberPrice !== undefined ? `Join — ₺${event.memberPrice} (member)` : 'Join this event'}
                  </button>
                  <button className="btn-secondary w-full justify-center text-sm py-2.5 mb-3">
                    Save for later
                  </button>
